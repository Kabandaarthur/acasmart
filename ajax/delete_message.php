...
// Soft-delete: replace content with placeholder and mark updated time
$placeholder = '[message deleted]';
$stmt_upd = $conn->prepare("UPDATE messages SET content = ? WHERE id = ?");
if ($stmt_upd) {
    $stmt_upd->bind_param('si', $placeholder, $message_id);
    if ($stmt_upd->execute()) {
        echo json_encode(['success' => true, 'id' => $message_id]);
    } else {
        $err = $conn->error ? $conn->error : 'Failed to delete';
        echo json_encode(['success' => false, 'message' => $err]);
    }
    $stmt_upd->close();
} else {
    $err = $conn->error ? $conn->error : 'Failed to prepare delete';
    echo json_encode(['success' => false, 'message' => $err]);
}
